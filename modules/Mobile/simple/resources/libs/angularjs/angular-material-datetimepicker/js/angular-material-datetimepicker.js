      ...
      function ($mdDialog) {
        return {
          ...
          link: function (scope, element, attrs, ngModel) {
            ...
            if (
              angular.isString(scope.currentDate) &&
              scope.currentDate !== ''
            ) {
  ...
  PluginController.prototype = {
      ...
      var _dateParam = function (input, fallback) {
        ...
        if (angular.isDefined(input) && input !== null && input !== '') {
          if (angular.isString(input)) {
            if (
              typeof that.params.format !== 'undefined' &&
              that.params.format !== null
            ) {
    ...
    isPreviousMonthVisible: function () {
      return (
        this.calendarStart &&
        this.isAfterMinDate(
          moment(this.calendarStart).startOf('month'),
          false,
          false,
        )
      )
    ...
    isNextMonthVisible: function () {
      return (
        ...
        this.isBeforeMaxDate(
          moment(this.calendarStart).endOf('month'),
          false,
          false,
        )
      )
    ...
    isPreviousYearVisible: function () {
      return (
        this.calendarStart &&
        this.isAfterMinDate(
          moment(this.calendarStart).startOf('year'),
          false,
          false,
        )
      )
    ...
    isNextYearVisible: function () {
      return (
        this.calendarStart &&
        this.isBeforeMaxDate(
          moment(this.calendarStart).endOf('year'),
          false,
          false,
        )
      )
    ...
    isHourAvailable: function (hour) {
      ...
      return (
        this.isAfterMinDate(_date, true, false) &&
        this.isBeforeMaxDate(_date, true, false)
      )
    },
    isMinuteAvailable: function (minute) {
      ...
      return (
        this.isAfterMinDate(_date, true, true) &&
        this.isBeforeMaxDate(_date, true, true)
      )
    },
    start: function () {
      this.currentView = VIEW_STATES.DATE
          ...
          controller: [
            ...
            function ($scope) {
              ...
              if (!picker.maxDate && !picker.minDate) {
                ...
                var high = picker.maxDate
                  ? currentMonthIndex(picker.maxDate) + 1
                  : MONTHS_IN_ALL
              ...
              calendar.topIndex =
                currentMonthIndex(picker.currentDate) - calendar.months[0]
              var generateMonthCalendar = function (date) {
                ...
                if (date !== null) {
                  ...
                  month.days = []
                  for (
                    var i = startOfMonth.date();
                    i <= startOfMonth.daysInMonth();
                    i++
                  ) {
                    ...
                    month.days.push(
                      moment(startOfMonth).locale(picker.params.lang).date(i),
                    )
                  }
                  var daysInAWeek = 7,
                    daysTmp = [],
                  ...
                  for (var j = 0; j < slices; j++) {
                    daysTmp.push(
                      month.days.slice(j * daysInAWeek, (j + 1) * daysInAWeek),
                    )
                  }
                  month.days = daysTmp
                  return month
              ...
              calendar.isInRange = function (date) {
                return (
                  picker.isAfterMinDate(moment(date), false, false) &&
              ...
              calendar.isSelectedDay = function (m) {
                return (
                  m &&
                  calendar.date.date() === m.date() &&
                  calendar.date.month() === m.month() &&
                  calendar.date.year() === m.year()
                )
              }
            },
          ],
          template:
            '<md-virtual-repeat-container class="months"><div md-virtual-repeat="idx in cal.months" md-start-index="cal.topIndex" md-item-size="' +
            ITEM_HEIGHT +
            '">     <div mdc-datetime-picker-calendar-month idx="idx"></div></div></md-virtual-repeat-container>',
        }
      },
    ])
    .directive('mdcDatetimePickerCalendarMonth', [
      '$compile',
      function ($compile) {
            ...
            weekDays.forEach(function (weekDay, j) {
              ...
              if (weekDay) {
                ...
                if (calendar.isInRange(weekDay)) {
                  ...
                  aOrSpan = angular
                    .element("<a href='#' mdc-dtp-noclick></a>")
                    .attr(
                      'ng-class',
                      '{selected: cal.isSelectedDay(' + scopeRef + ')}',
                    )
                    .attr('ng-click', 'cal.selectDate(' + scopeRef + ')')
                } else {
                  aOrSpan = angular.element('<span></span>')
                }
                aOrSpan.addClass('dtp-select-day').html(weekDay.format('D'))
                td.append(aOrSpan)
              }
              tr.append(td)
            })
        ...
        return {
          ...
          restrict: 'AE',
          template:
            '<div class="dtp-picker-month">{{month.name}}</div><table class="table table-stripeddtp-picker-days">    <thead>    <tr>        <th ng-repeat="day in cal.week">{{cal.toDay(day)}}</th>    </tr>    </thead>    <tbody>    </tbody></table>',
    ...
    function () {
      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          var exec = function () {
            var clock = angular.element(
                element[0].querySelector('.dtp-picker-clock'),
              ),
              pickerEl = angular.element(
                componentRoot.querySelector('.dtp-picker'),
              )
            var w = componentRoot.querySelector('.dtp-content').offsetWidth
            var pl =
              parseInt(css(pickerEl, 'paddingLeft').replace('px', '')) || 0
            var pr =
              parseInt(css(pickerEl, 'paddingRight').replace('px', '')) || 0
            ...
            var pL =
              parseInt(css(pickerEl, 'paddingLeft').replace('px', '')) || 0
            var pT =
              parseInt(css(pickerEl, 'paddingTop').replace('px', '')) || 0
            var mL = parseInt(css(clock, 'marginLeft').replace('px', '')) || 0
            ...
            var _mL = r / 1.5
            angular
              .element(element[0].querySelector('.dtp-hour-hand'))
              .css({
                left: r + mL * 1.5 + 'px',
                height: _hL + 'px',
            ...
            angular
              .element(element[0].querySelector('.dtp-minute-hand'))
              .css({
                left: r + mL * 1.5 + 'px',
                height: _mL + 'px',
          ...
          var animateHands = function () {
            ...
            rotateElement(
              angular.element(element[0].querySelector('.dtp-minute-hand')),
              mdg,
            )
          }
          ...
          if (!picker.params.shortTime) {
            scope.$watch('picker.meridien', function () {
              if (!minuteMode) {
                if (scope.points) {
                  var points = setDisplayPoints(
                    picker.isPM(),
                    angular.copy(scope.points),
                  )
                  scope.points = points
                }
              }
          ...
          scope.pointAvailable = function (point) {
            return minuteMode
              ? picker.isMinuteAvailable(point.value)
              : picker.isHourAvailable(point.value)
          }
