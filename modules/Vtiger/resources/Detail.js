...
Vtiger.Class(
  ...
  },
  {
    ...
    registerRelatedListPageNavigationEvents: function () {
      ...
      detailViewContainer.on('click', '#listViewPreviousPageButton', function (
        e,
      ) {
        var pageNumber = jQuery('#pageNumber').val()
        if (pageNumber > 1) {
    ...
    ajaxEditHandling: function (currentTdElement) {
      ...
      if (
        ...
        var sourcePicklistFieldName = thisInstance.getDependentSourcePicklistName(
          fieldName,
        )
    ...
    getDependentSourcePicklistName: function (fieldName) {
      ...
      jQuery.each(picklistDependencyMapping, function (
        sourcePicklistName,
        configuredDependencyObject,
      ) {
        var picklistmap = configuredDependencyObject['__DEFAULT__']
        jQuery.each(picklistmap, function (
          targetPickListName,
    ...
    registerSummaryViewContainerEvents: function (summaryViewContainer) {
      ...
      jQuery('.createRecord').on('click', function (e) {
        ...
        var widgetHeaderContainer = summaryWidgetContainer.find(
          '.widget_header',
        )
    ...
    toggleCommentContent: function (e) {
      ...
      var toggleElement = jQuery(
        '<div><a class="pull-right toggleComment" style="color: 38a767;"><small></small></a><div>',
      )
    ...
    registerScrollForRollupEvents: function () {
      ...
      if (relatedController) relatedController.registerScrollForRollupComments()
    },
    ...
    registerTagEvents: function () {
      ...
      app.event.on('post.MassTag.save', function (
        e,
        modalContainerClone,
        data,
      ) {
    ...
    registerRelatedRowClickEvent: function () {
      var detailContentsHolder = this.getContentHolder()
      detailContentsHolder.on('click', '.relatedListEntryValues a', function (
        e,
      ) {
        e.preventDefault()
      })
    ...
    registerEvents: function () {
      ...
      detailContentsHolder.on('click', '.saveComment', function (e) {
        ...
        if (!element.is(':disabled')) {
          ...
          var dataObj = self.saveComment(e)
          dataObj.then(function (data) {
            var closestAddCommentBlock = currentTarget.closest(
              '.addCommentBlock',
            )
            var commentTextAreaElement = closestAddCommentBlock.find(
              '.commentcontent',
            )
            ...
            if (mode == 'add') {
              ...
              commentHtml.then(function (data) {
                ...
                var commentBlock = closestAddCommentBlock.closest(
                  '.commentDetails',
                )
      ...
      app.event.on('post.summarywidget.load', function (
        ...
        if (
          ...
          widgetContainer
            .off('switchChange.bootstrapSwitch')
            .on('switchChange.bootstrapSwitch', '#rollupcomments', function (
              e,
            ) {
              app.helper.showProgress()
              self.toggleRollupComments(e)
    ...
    registerPostAjaxSaveEvent: function () {
      var _this = this
      app.event.on(Vtiger_Detail_Js.PostAjaxSaveEvent, function (
        e,
        fieldBasicData,
        postSaveRecordDetails,
        contentHolder,
