...
Vtiger_Edit_Js(
  'Inventory_Edit_Js',
  {
    ...
    lineItemPopOverTemplate:
      '<div class="popover lineItemPopover" role="tooltip"><div class="arrow"></div>\n\
                                <h3 class="popover-title"></h3>\n\
								<div class="popover-content"></div>\n\
									<div class="modal-footer lineItemPopupModalFooter">\n\
										<center>\n\
										<button class="btn btn-success popoverButton" type="button"><strong>' +
      app.vtranslate('JS_LBL_SAVE') +
      '</strong></button>\n\
										<a href="#" class="popoverCancel" type="reset">' +
      app.vtranslate('JS_LBL_CANCEL') +
      '</a>\n\
										</center>\n\
									</div>\n\
                                </div>',
  },
  {
    ...
    initializeVariables: function () {
      ...
      this.numOfLineItems = this.lineItemsHolder.find(
        '.' + this.lineItemDetectingClass,
      ).length
      ...
      if (typeof jQuery('#customFieldsDefaultValues').val() == 'undefined') {
        ...
        this.customFieldsDefaultValues = JSON.parse(
          jQuery('#customFieldsDefaultValues').val(),
        )
      }

      this.numOfCurrencyDecimals = parseInt(
        jQuery('.numberOfCurrencyDecimal').val(),
      )
      ...
      this.getRecordDetails(data).then(
        function (data) {
          ...
          if (typeof addressMap != 'undefined') {
            ...
            self.mapAddressDetails(
              self.addressFieldsMapping[sourceModule],
              response['data'],
              container,
            )
            if (sourceModule == 'Accounts') {
    ...
    copyAddress: function (swapMode) {
      ...
      if (swapMode == 'false') {
        for (var key in addressMapping) {
          ...
          var toElement = formElement.find(
            '[name="' + addressMapping[key] + '"]',
          )
    ...
    formatListPrice: function (lineItemRow, listPriceValue) {
      var listPrice = parseFloat(listPriceValue).toFixed(
        this.numOfCurrencyDecimals,
      )
    ...
    setPurchaseCostValue: function (lineItemRow, purchaseCost) {
      ...
      lineItemRow
        .find('[name="purchaseCost' + rowNum + '"]')
        .val(updatedPurchaseCost)
    ...
    getLineItemTotal: function (lineItemRow) {
      ...
      if (lineItemTotal) return parseFloat(lineItemTotal)
      return 0
    },
    ...
    getLineItemTaxTotal: function (lineItemRow) {
      ...
      if (lineItemTax) return parseFloat(lineItemTax)
      return 0
    },
    ...
    formatLineItemNetPrice: function (netPriceEle) {
      ...
      if (lineItemNetPrice) return parseFloat(lineItemNetPrice)
      return 0
    },
    ...
    getNetTotal: function () {
      ...
      if (netTotal) return parseFloat(netTotal)
      return 0
    },
    ...
    getFinalDiscountTotal: function () {
      ...
      if (discountTotal) return parseFloat(discountTotal)
      return 0
    },
    ...
    getGroupTaxTotal: function () {
      ...
      if (groupTax) return parseFloat(groupTax)
      return 0
    },
    ...
    getGrandTotal: function () {
      ...
      if (grandTotal) return parseFloat(grandTotal)
      return 0
    },

    isIndividualTaxMode: function () {
      return this.taxTypeElement.val() == Inventory_Edit_Js.individualTaxType
        ? true
        : false
    },

    isGroupTaxMode: function () {
      return this.taxTypeElement.val() == Inventory_Edit_Js.groupTaxType
        ? true
        : false
    },
    ...
    checkLineItemRow: function () {
      var numRows = this.lineItemsHolder.find('.' + this.lineItemDetectingClass)
        .length
      if (numRows > 1) {
    ...
    saveProductCount: function () {
      jQuery('#totalProductCount').val(
        this.lineItemsHolder.find('tr.' + this.lineItemDetectingClass).length,
      )
    },
    ...
    updateRowNumberForRow: function (
      lineItemRow,
      ...
      for (var idIndex in idFields) {
        ...
        lineItemRow
          .find('#' + actualElementId)
          .attr('id', expectedElementId)
          .filter('[name="' + actualElementId + '"]')
      ...
      for (var nameIndex in nameFields) {
        ...
        lineItemRow
          .find('[name="' + actualElementName + '"]')
          .attr('name', expectedElementName)
      }

      lineItemRow
        .attr('id', expectedRowId)
        .attr('data-row-num', expectedSequenceNumber)
    ...
    updateLineItemElementByOrder: function () {
      ...
      var lineItems = this.lineItemsHolder.find(
        'tr.' + this.lineItemDetectingClass,
      )
      lineItems.each(function (index, domElement) {
        ...
        var element = discountContianer
          .find('input.discounts')
          .filter(':checked')
      ...
      lineItems.each(function (index, domElement) {
        ...
        if (expectedRowId != actualRowId) {
          ...
          self.updateRowNumberForRow(
            lineItemRow,
            expectedRowIndex,
            actualIdComponents[1],
          )

          var discountContianer = lineItemRow.find('div.discountUI')
          discountContianer
            .find('input.discounts')
            .each(function (index1, discountElement) {
    ...
    getNewLineItem: function (params) {
      ...
      var newRow = this.dummyLineItemRow
        .clone(true)
        .removeClass('hide')
        .addClass(this.lineItemDetectingClass)
        .removeClass('lineItemCloneCopy')
      ...
      newRow
        .find('.lineItemPopup')
        .filter(':not([data-module-name="' + itemType + '"])')
        .remove()
    ...
    calculateLineItemTotal: function (lineItemRow) {
      ...
      this.setLineItemTotal(
        lineItemRow,
        lineItemTotal.toFixed(this.numOfCurrencyDecimals),
      )
    },
    ...
    calculateDiscountForLineItem: function (lineItemRow) {
      ...
      jQuery('.productTotalVal', lineItemRow).text(
        productTotal.toFixed(this.numOfCurrencyDecimals),
      )
      this.setDiscountTotal(
        lineItemRow,
        parseFloat(discountValue).toFixed(this.numOfCurrencyDecimals),
    ...
    calculateTotalAfterDiscount: function (lineItemRow) {
      ...
      totalAfterDiscount = totalAfterDiscount.toFixed(
        this.numOfCurrencyDecimals,
      )
      ...
      jQuery.each(taxPercentages, function (index, domElement) {
        ...
        if (isNaN(individualTaxPercentage)) {
          ...
          var individualTaxTotal =
            Math.abs(individualTaxPercentage * totalAfterDiscount) / 100
          individualTaxTotal = individualTaxTotal.toFixed(
            self.numOfCurrencyDecimals,
          )
        }
        ...
        if (compoundOn) {
          ...
          jQuery.each(compoundOn, function (index, id) {
            if (!isNaN(jQuery('.taxTotal' + id, lineItemRow).val())) {
              amount =
                parseFloat(amount) +
                parseFloat(jQuery('.taxTotal' + id, lineItemRow).val())
            }
          })
    ...
    calculateNetTotal: function () {
      var self = this
      var netTotalValue = 0
      this.lineItemsHolder
        .find('tr.' + this.lineItemDetectingClass + ' .netPrice')
    ...
    calculateFinalDiscount: function () {
      ...
      var element = discountContainer
        .find('input.finalDiscounts')
        .filter(':checked')
      var discountType = element.data('discountType')
      var discountRow = element.closest('tr')
      ...
      var rowPercentageField = discountContainer.find(
        'input.discount_percentage_final',
      )
      ...
      if (discountType == Inventory_Edit_Js.percentageDiscountType) {
        ...
        if (this.prevSelectedCurrencyConversionRate) {
          ...
          conversionRate =
            conversionRate / this.prevSelectedCurrencyConversionRate
    ...
    calculatePreTaxTotal: function () {
      ...
      if (this.isGroupTaxMode()) {
        ...
        var netTotal = 0
        var elementsList = this.lineItemsHolder.find(
          '.' + this.lineItemDetectingClass,
        )
    ...
    calculateCharges: function () {
      ...
      var netTotal = this.getNetTotal()
      var finalDiscountValue = this.getFinalDiscountTotal()
      var amount = parseFloat(netTotal - finalDiscountValue).toFixed(
        numberOfDecimal,
      )

      chargesBlockContainer
        .find('.chargePercent')
        .each(function (index, domElement) {
          ...
          element
            .closest('tr')
            .find('.chargeValue')
            .val(parseFloat(value).toFixed(numberOfDecimal))
        })
      ...
      chargesBlockContainer
        ...
        .each(function (index, domElement) {
          ...
          chargesTotal =
            parseFloat(chargesTotal) + parseFloat(chargeElementValue)
        })

      this.chargesTotalEle.val(parseFloat(chargesTotal))
      this.chargesTotalDisplay.text(
        parseFloat(chargesTotal).toFixed(numberOfDecimal),
      )
    ...
    calculateChargeTaxes: function () {
      ...
      chargesBlockContainer
        .find('.chargeTaxPercentage')
        .each(function (index, domElement) {
          var element = jQuery(domElement)
          var chargeId = element.data('chargeId')
          var chargeAmount = self.chargesContainer
            .find('[name="charges[' + chargeId + '][value]"]')
            .val()
          if (isNaN(element.val())) {
            var value = 0
          } else {
          ...
          element
            .closest('tr')
            .find('.chargeTaxValue')
            .val(parseFloat(value).toFixed(self.numOfCurrencyDecimals))
        })
        ...
        .each(function (index, domElement) {
          ...
          if (compoundOn) {
            ...
            var chargeAmount = parseFloat(
              self.chargesContainer
                .find('[name="charges[' + chargeId + '][value]"]')
                .val(),
            ).toFixed(self.numOfCurrencyDecimals)

            jQuery.each(compoundOn, function (index, id) {
              var chargeTaxEle = chargesBlockContainer.find(
                '.chargeTax' + chargeId + id,
              )
              if (!isNaN(chargeTaxEle.val())) {
                chargeAmount =
                  parseFloat(chargeAmount) + parseFloat(chargeTaxEle.val())
              }
            })
      ...
      jQuery('#chargeTaxTotal').text(
        parseFloat(chargesTotal).toFixed(this.numOfCurrencyDecimals),
      )
      this.chargeTaxesTotal.val(
        parseFloat(chargesTotal).toFixed(this.numOfCurrencyDecimals),
      )
      this.calculatePreTaxTotal()
      this.calculateGrandTotal()
    ...
    calculateGrandTotal: function () {
      ...
      var grandTotal =
        ...
        parseFloat(shippingHandlingCharge) +
        parseFloat(shippingHandlingTax) -
        parseFloat(deductedTaxesAmount)
    ...
    calculateGroupTax: function () {
      ...
      var groupTaxTotal = 0
      this.groupTaxContainer
        .find('.groupTaxPercentage')
        .each(function (index, domElement) {
          ...
          if (isNaN(groupTaxPercentageElement.val())) {
            ...
            var groupTaxValue =
              Math.abs(amount * groupTaxPercentageElement.val()) / 100
          }
          groupTaxValue = parseFloat(groupTaxValue).toFixed(
            self.numOfCurrencyDecimals,
          )
        ...
        .each(function (index, domElement) {
          ...
          if (compoundOn) {
            ...
            jQuery.each(compoundOn, function (index, value) {
              var groupTaxAmountValue = self.groupTaxContainer
                .find('[name="tax' + value + '_group_amount"]')
                .val()
              if (!isNaN(groupTaxAmountValue)) {
                totalAmount =
                  parseFloat(totalAmount) + parseFloat(groupTaxAmountValue)
              }
            })
            ...
            if (isNaN(groupTaxPercentageElement.val())) {
              ...
              var groupTaxValue =
                Math.abs(totalAmount * groupTaxPercentageElement.val()) / 100
            }
    ...
    calculateDeductTaxes: function () {
      ...
      this.dedutTaxesContainer
        .find('.deductTaxPercentage')
        .each(function (index, domElement) {
          var value = 0
          var element = jQuery(domElement)
          if (!isNaN(element.val())) {
          ...
          deductTaxesTotalAmount =
            parseFloat(deductTaxesTotalAmount) + parseFloat(value)
        })

      this.deductTaxesTotal.text(
        parseFloat(deductTaxesTotalAmount).toFixed(this.numOfCurrencyDecimals),
      )
      this.calculateGrandTotal()
    },
    ...
    lineItemDirectDiscountCal: function (conversionRate) {
      ...
      self.lineItemsHolder
        .find('tr.' + self.lineItemDetectingClass)
        .each(function (index, domElement) {
          var lineItemRow = jQuery(domElement)
          var discountContianer = lineItemRow.find('div.discountUI')
          var element = discountContianer
          ...
          if (discountType == Inventory_Edit_Js.directAmountDiscountType) {
            ...
            self.setDiscountTotal(
              lineItemRow,
              newdiscountValue.toFixed(self.numberOfCurrencyDecimals),
            )
          }
        })
    },
    ...
    AdjustmentShippingResultCalculation: function (conversionRate) {
      ...
      chargesBlockContainer
        ...
        .each(function (index, domElement) {
          ...
          jQuery(chargeElement).val(
            parseFloat(jQuery(domElement).val()) * conversionRate,
          )
        })
    ...
    loadSubProducts: function (lineItemRow) {
      ...
      app.request.get({ data: subProrductParams }).then(function (error, data) {
        if (!data) {
          return
        }
        var result = data
        ...
        for (var id in responseData) {
          if (isBundleViewable == 1) {
            subProductHtml +=
              '<em> - ' +
              responseData[id]['productName'] +
              ' (' +
              responseData[id]['quantity'] +
              ')'
            if (responseData[id]['stockMessage']) {
              subProductHtml +=
                ' - <span class="redColor">' +
                responseData[id]['stockMessage'] +
                '</span>'
            }
    ...
    getTaxDiv: function (taxObj, parentRow) {
      ...
      if (!jQuery.isEmptyObject(taxObj)) {
        taxDiv +=
          ...
          '<table width="100%" border="0" cellpadding="5" cellspacing="0" class="table table-stripedtable-nobordered popupTable" id="tax_table' +
          rowNumber +
          '">'
        ...
        for (var taxName in taxObj) {
          ...
          taxDiv +=
            ...
            '_percentage' +
            rowNumber +
            '" data-rule-positive=true data-rule-inventory_percentage=true  id="' +
            taxName +
            '_percentage' +
            rowNumber +
            '" value="' +
            taxInfo.taxpercentage +
            '" class="taxPercentage" data-compound-on=' +
            taxInfo.compoundOn +
            ' data-regions-list="' +
            taxInfo.regionsList +
            '">&nbsp;%' +
            ...
            '<input type="text" name="popup_tax_row' +
            rowNumber +
            '" class="cursorPointer span1 taxTotal taxTotal' +
            taxInfo.taxid +
            '" value="0.0" readonly>' +
            '</td>' +
            '</tr>'
        ...
        taxDiv +=
          ...
          '<span>' +
          app.vtranslate('JS_NO_TAXES_EXISTS') +
          '</span>' +
          '</div>'
      }
    ...
    mapResultsToFields: function (parentRow, responseData) {
      ...
      for (var id in responseData) {
        ...
        jQuery('input.listPrice', parentRow).attr(
          'list-info',
          listPriceValuesJson,
        )
        jQuery('input.listPrice', parentRow).data(
          'baseCurrencyId',
          recordData.baseCurrencyId,
        )
    ...
    postLineItemSelectionActions: function (
      itemRow,
      selectedLineItemsData,
      lineItemSelectedModuleName,
    ) {
    ...
    pricebooksPopupHandler: function (popupImageElement) {
      var self = this
      var lineItemRow = popupImageElement.closest(
        'tr.' + this.lineItemDetectingClass,
      )
      var lineItemProductOrServiceElement = lineItemRow
        .find('input.productName')
        .closest('td')
      var params = {}
      params.module = 'PriceBooks'
      params.src_module = lineItemProductOrServiceElement
        .find('i.lineItemPopup')
        .data('moduleName')
      params.src_field = lineItemProductOrServiceElement
        .find('i.lineItemPopup')
        .data('fieldName')
      params.src_record = lineItemProductOrServiceElement
        .find('input.selectedModuleId')
        .val()
    ...
    registerProductAndServiceSelector: function () {
      ...
      this.lineItemsHolder.on('click', '.lineItemPopup', function (e) {
        ...
        var postPopupHandler = function (e, data) {
          ...
          self.postLineItemSelectionActions(
            triggerer.closest('tr'),
            data,
      ...
      this.lineItemsHolder.on('focusout', 'input.listPrice', function (e) {
        ...
        if (isPriceChanged == false) {
          ...
          conversionRate =
            ...
            parseFloat(prevSelectedCurrencyConversionRate)
          var convertedListPrice = listPriceValues[currencyId]
          if (typeof listPriceValues[currencyId] == 'undefined') {
            var baseCurrencyId = element.data('baseCurrencyId')
            var baseCurrencyElement = currencyElement.find(
              "option[value='" + baseCurrencyId + "']",
            )
            convertedListPrice =
              (listPriceValues[baseCurrencyId] *
                optionsSelected.data('conversionRate')) /
              baseCurrencyElement.data('conversionRate')
          }
    ...
    registerPriceBookPopUp: function () {
      ...
      this.lineItemsHolder.on('click', '.priceBookPopup', function (e) {
        ...
        app.event.one(
          'post.LineItemPriceBookSelect.click',
          postPriceBookPopupHandler,
        )
      })
    },
    ...
    registerLineItemTaxShowEvent: function () {
      ...
      this.lineItemsHolder.on('click', '.individualTax', function (e) {
        ...
        var taxUI = parentElem
          .find('div.taxUI')
          .clone(true, true)
          .removeClass('hide')
          .addClass('show')
        taxUI.find('div.individualTaxDiv').removeClass('hide').addClass('show')
        var popOverTitle = taxUI
          .find('.popover_title')
          .find('.variable')
          .text(self.getTotalAfterDiscount(lineItemRow))
          .closest('.popover_title')
          .text()
        ...
        element.one('shown.bs.popover', function (e) {
          ...
          if (
            element.next('.popover').find('.popover-content').height() > 300
          ) {
            app.helper.showScroll(
              element.next('.popover').find('.popover-content'),
              { height: '300px' },
            )
          }
        })
        ...
        if (self.isIndividualTaxMode()) {
          ...
          self.lineItemsHolder
            ...
            .each(function (index, domElement) {
              var lineItemRow = jQuery(domElement)
              lineItemRow
                .find('.individualTaxContainer,.productTaxTotal')
                .removeClass('hide')
    ...
    registerCurrencyChangeEvent: function () {
      ...
      this.currencyElement.change(function (e) {
        ...
        self.lineItemsHolder
          ...
          .each(function (index, domElement) {
            var lineItemRow = jQuery(domElement)
            var isLineItemSelected = jQuery(lineItemRow)
              .find('.selectedModuleId')
              .val()
            ...
            var updatedPurchaseCost =
              parseFloat(purchaseCostVal) * parseFloat(conversionRate)
            ...
            if (isPriceChanged == false) {
              ...
              if (typeof listPriceValues[currencyId] == 'undefined') {
                ...
                var baseCurrencyElement = element.find(
                  "option[value='" + baseCurrencyId + "']",
                )
                convertedListPrice =
                  (listPriceValues[baseCurrencyId] *
      ...
      this.lineItemsHolder.on('click', '.individualDiscount', function (e) {
        ...
        var callBackFunction = function (element, data) {
          var triggerDiscountChangeEvent = function (discountDiv) {
            var selectedDiscountType = discountDiv
              .find('input.discounts')
              .filter(':checked')
            ...
            if (discountType == Inventory_Edit_Js.percentageDiscountType) {
              ...
              discountType == Inventory_Edit_Js.directAmountDiscountType
            ) {
          ...
          data.find('.popoverButton').on('click', function (e) {
            ...
            if (validate) {
              ...
              if (discountType == Inventory_Edit_Js.percentageDiscountType) {
                jQuery('input.discount_percentage', oldDiscountDiv).val(
                  discountValue,
                )
              } else if (
                discountType == Inventory_Edit_Js.directAmountDiscountType
              ) {
                jQuery('input.discount_amount', oldDiscountDiv).val(
                  discountValue,
                )
              }
              element.popover('destroy')
    ...
    registerFinalDiscountShowEvent: function () {
      ...
      var popOverTemplate = jQuery(Inventory_Edit_Js.lineItemPopOverTemplate)
        .css('opacity', 0)
        .css('z-index', '-1')
      ...
      this.finalDiscountEle.on('click', function (e) {
        ...
        if (popOverEle.css('opacity') == '0') {
          ...
          popOverEle
            .find('.popover-title')
            .text(popOverEle.find('.popover_title').text())
    ...
    registerFinalDiscountChangeEvent: function () {
      ...
      this.finalDiscountUIEle.on('change', '.finalDiscounts', function (e) {
        ...
        var rowAmountField = discountContainer.find(
          'input.discount_amount_final',
        )
    ...
    registerChargeBlockShowEvent: function () {
      ...
      chargesTrigger.on('shown.bs.popover', function () {
        if (
          chargesTrigger.next('.popover').find('.popover-content').height() >
          300
        ) {
          app.helper.showScroll(
            chargesTrigger.next('.popover').find('.popover-content'),
            { height: '300px' },
          )
        }
    ...
    registerChargeBlockChangeEvent: function () {
      ...
      chargesBlockContainer.on(
        'focusout',
        '.chargePercent,.chargeValue',
        function (e) {
          var element = jQuery(e.currentTarget)
          if (element.closest('form').valid()) {
            self.calculateCharges()
    ...
    registerGroupTaxShowEvent: function () {
      ...
      var finalTaxUI = jQuery('#group_tax_row')
        .find('.finalTaxUI')
        .removeClass('hide')
      ...
      finalTaxTriggerer.on('shown.bs.popover', function () {
        var finalTaxForm = jQuery('#group_tax_row')
          .find('.finalTaxUI')
          .closest('.lineItemPopover')
        if (
          finalTaxTriggerer.next('.popover').find('.popover-content').height() >
          300
        ) {
          app.helper.showScroll(
            finalTaxTriggerer.next('.popover').find('.popover-content'),
            { height: '300px' },
          )
        }
    ...
    registerChargeTaxesShowEvent: function () {
      ...
      chargeTaxTriggerer.on('shown.bs.popover', function () {
        if (
          ...
          app.helper.showScroll(
            chargeTaxTriggerer.next('.popover').find('.popover-content'),
            { height: '300px' },
          )
        }
        var chargesTaxForm = self.chargeTaxesContainer.closest(
          '.lineItemPopover',
        )
    ...
    registerDeductTaxesShowEvent: function () {
      ...
      deductTaxesTriggerer.on('shown.bs.popover', function () {
        if (
          deductTaxesTriggerer
            .next('.popover')
            .find('.popover-content')
            .height() > 300
        ) {
          app.helper.showScroll(
            deductTaxesTriggerer.next('.popover').find('.popover-content'),
            { height: '300px' },
          )
        }
    ...
    registerDeductTaxesChangeEvent: function () {
      ...
      this.dedutTaxesContainer.on('focusout', '.deductTaxPercentage', function (
        e,
      ) {
        ...
        app.helper.showConfirmationBox({ message: message }).then(
          function (e) {
            ...
            for (var taxId in taxes) {
              element = self.groupTaxContainer.find(
                '[name="tax' + taxId + '_group_percentage"]',
              )
            ...
            for (var chargeId in charges) {
              ...
              if (chargeInfo.hasOwnProperty('value')) {
                property = 'value'
                chargeValue =
                  parseFloat(chargeInfo[property]) *
                  parseFloat(jQuery('#conversion_rate').val())
              }
              self.chargesContainer
                .find('[name="charges[' + chargeId + '][' + property + ']"]')
                .val(chargeValue)

              var chargeTaxes = chargeInfo['taxes']
              for (var chargeTaxId in chargeTaxes) {
                element = self.chargeTaxesContainer.find(
                  '[name="charges[' +
                    chargeId +
                    '][taxes][' +
                    chargeTaxId +
                ...
                element.data(
                  'compoundOn',
                  chargeTaxes[chargeTaxId]['compoundOn'],
                )
              }
            }
            self.calculateCharges()
    ...
    registerSubmitEvent: function () {
      ...
      editViewForm.submit(function (e) {
        ...
        var elementsList = self.lineItemsHolder.find(
          '.' + self.lineItemDetectingClass,
        )
        //			jQuery.each(elementsList, function(index, element) {
        //				var lineItemRow = jQuery(element);
    ...
    makeLineItemsSortable: function () {
      ...
      this.lineItemsHolder
        ...
        .mousedown(function (event) {
          ...
          self
            .getClosestLineItemRow(jQuery(event.target))
            .find('input:focus')
            .trigger('focusout')
        })
    },
    ...
    registerEventForCopyAddress: function () {
      var self = this
      jQuery(
        '[name="copyAddressFromRight"],[name="copyAddressFromLeft"]',
      ).change(function () {
        ...
        if (elementClass == 'accountAddress') {
          ...
          if (typeof recordRelativeAccountId == 'undefined') {
            app.helper.showErrorNotification({
              message: app.vtranslate('JS_RELATED_ACCOUNT_IS_NOT_AVAILABLE'),
            })
            return
          }
          if (recordRelativeAccountId == '' || recordRelativeAccountId == '0') {
            ...
            if (targetCopyAddress == 'billing') {
              objectToMapAddress =
                self.addressFieldsMappingBetweenModules['AccountsBillMap']
            } else if (targetCopyAddress == 'shipping') {
              objectToMapAddress =
                self.addressFieldsMappingBetweenModules['AccountsShipMap']
            }
          ...
          if (typeof recordRelativeContactId == 'undefined') {
            app.helper.showErrorNotification({
              message: app.vtranslate('JS_RELATED_CONTACT_IS_NOT_AVAILABLE'),
            })
            return
          }
          if (recordRelativeContactId == '' || recordRelativeContactId == '0') {
            ...
            if (targetCopyAddress == 'billing') {
              objectToMapAddress =
                self.addressFieldsMappingBetweenModules[
                  editViewSelection + 'BillMap'
                ]
            } else if (targetCopyAddress == 'shipping') {
              objectToMapAddress =
                self.addressFieldsMappingBetweenModules[
                  editViewSelection + 'ShipMap'
                ]
            }
            self.copyAddressDetails(
              data,
    ...
    registerForTogglingBillingandShippingAddress: function () {
      var billingAddressPosition = jQuery('[name="bill_street"]')
        .closest('td')
        .index()
      ...
      var copyAddress1toggleAddressLeftContainer = copyAddress1Block.find(
        '[name="togglingAddressContainerLeft"]',
      )
      var copyAddress1toggleAddressRightContainer = copyAddress1Block.find(
        '[name="togglingAddressContainerRight"]',
      )
      var copyAddress2toggleAddressLeftContainer = copyAddress2Block.find(
        '[name="togglingAddressContainerLeft"]',
      )
      var copyAddress2toggleAddressRightContainer = copyAddress2Block.find(
        '[name="togglingAddressContainerRight"]',
      )
      ...
      container.find('input.autoComplete').autocomplete({
        ...
        select: function (event, ui) {
          ...
          var dataUrl =
            ...
            selectedItemData.id +
            '&currency_id=' +
            jQuery('#currency_id option:selected').val() +
            '&sourceModule=' +
            app.getModuleName()
    ...
    registerReferenceSelectionEvent: function (container) {
      ...
      jQuery('input[name="contact_id"]', container).on(
